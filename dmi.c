...
/* Select internal DMI decoder instead of relying on dmidecode */
#define DMI_INTERNAL 1
...
#if DMI_INTERNAL
static int dmi_checksum(const uint8_t const *buf, size_t len)
{
	uint8_t sum = 0;
	size_t a;
...
static void dmi_table(uint32_t base, uint16_t len, uint16_t num)
{
	...
	//uint8_t *dmi_table_mem = rphysmap("DMI Table", base, len);
...
int dmi_fill(void)
{
	 ...
	 *   On linux it can be obtained from sysfs. EFI's SMBIOS GUID is:
	 *   {0xeb9d2d31,0x2d88,0x11d3,0x9a,0x16,0x0,0x90,0x27,0x3f,0xc1,0x4d}
	 * - Scanning physical memory address range 0x000F0000h to 0x000FFFFF
	 *   for the anchor-string(s). */
